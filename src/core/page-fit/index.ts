...
export { solve, interpolate } from '../../lib/solver/solve.js'
export type {
	TextMetricsSnapshot,
	TextBlockMetric,
	ElementCounts,
...
/**
 * Fit content into target pages on an already-loaded Playwright page.
 * The page must have viewport set and HTML content loaded before calling.
 * Caller manages browser/page lifecycle.
 */
export async function fitToPagesOnPage(
	page: Page,
	html: string,
	targetPages: number,
): Promise<FitResult> {
	...
	for (let i = 0; i < REFINE_ITERATIONS; i++) {
		...
		const blank = pageCapacity(current['page-margin-y'], targetPages) - contentH
	...
	if (currentPages <= targetPages) {
		...
		if (blank > TARGET_BLANK) {
			...
			allAdjustments['page-margin-y'] = formatVar('page-margin-y', newMarginIn)
			await applyVariables(page, allAdjustments)
...
/**
 * Convenience wrapper that manages browser lifecycle via the shared pool.
 * Used by the CLI render pipeline.
 */
export async function fitToPages(
	...
	try {
		...
		try {
			...
			return await fitToPagesOnPage(page, html, targetPages)
		} finally {
			await page.close()
		}
	} finally {
