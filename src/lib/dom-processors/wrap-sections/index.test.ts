...
describe('wrapSections', () => {
	...
	describe('two-column layout handling', () => {
		it('wraps sections inside primary column', () => {
			const html = `<div class="two-column-layout">
				<div class="primary"><h2>Experience</h2><p>Job</p></div>
				<div class="secondary"><h2>Skills</h2><p>Skill</p></div>
			</div>`
			const result = wrapSections(html, createContext())
			const doc = parseHtml(result)

			// Root has two-column-layout
			const layout = doc.body.children[0] as Element
			expect(layout.getAttribute('class')).toBe('two-column-layout')

			// Primary column contains exactly one section
			const primary = layout.querySelector('.primary') as Element
			expect(primary.children.length).toBe(1)

			const section = primary.children[0] as Element
			expect(section.tagName).toBe('SECTION')
			...
			expect(section.children[1].tagName).toBe('P')
			expect(section.children[1].textContent).toBe('Job')
		})

		it('wraps sections inside secondary column', () => {
			const html = `<div class="two-column-layout">
				<div class="primary"><h2>Experience</h2><p>Job</p></div>
				<div class="secondary"><h2>Skills</h2><p>Skill</p></div>
			</div>`
			...
			const layout = doc.body.children[0] as Element
			const secondary = layout.querySelector('.secondary') as Element

			// Secondary column contains exactly one section
			expect(secondary.children.length).toBe(1)

			const section = secondary.children[0] as Element
			expect(section.tagName).toBe('SECTION')
			expect(section.getAttribute('id')).toBe('skills')
			expect(section.children.length).toBe(2)
			expect(section.children[0].tagName).toBe('H2')
			expect(section.children[0].textContent).toBe('Skills')
			expect(section.children[1].tagName).toBe('P')
			expect(section.children[1].textContent).toBe('Skill')
		})
		...
		it('handles multiple sections per column', () => {
			...
			// Primary has exactly 2 sections
			expect(primary.children.length).toBe(2)

			const section1 = primary.children[0] as Element
			const section2 = primary.children[1] as Element

			expect(section1.tagName).toBe('SECTION')
			expect(section1.getAttribute('id')).toBe('experience')
			expect(section1.children[0].textContent).toBe('Experience')
			expect(section1.children[1].textContent).toBe('Job')

			expect(section2.tagName).toBe('SECTION')
			expect(section2.getAttribute('id')).toBe('projects')
			expect(section2.children[0].textContent).toBe('Projects')
			expect(section2.children[1].textContent).toBe('Project')
		})
	})
